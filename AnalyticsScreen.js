...
const AnalyticsScreen = ({ stats, onFinish }) => {
  ...
  // Find top score ceiling boundary limit to draw chart heights proportionally
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      {/* --- KEY ADMINISTRATIVE ENGAGEMENT COUNTERS --- */}
      ...
      <View style={styles.chartCard}>
        ...
        {chartData.posts && chartData.posts.length > 0 ? (
          <View style={styles.barGraphContainer}>
            {chartData.posts.map((post, index) => {
              // Mathematical factor map scales bars proportionally from 15% to 100% height
              const barHeightPercentage = `${Math.max(15, ((post.votes || 0) / maxVotes) * 100)}%`;
              const shortText = post.type === 'text' 
                ? (post.content.length > 12 ? `${post.content.substring(0, 12)}...` : post.content)
        ...
        <View style={styles.legendRow}>
          <Text style={styles.legendText}>ℹ️ Graf di atas menunjukkan susunan populariti berdasarkan bilangan undian maklum balas pelajar.</Text>
        </View>
      </View>

      {/* --- FORMAT PROFILE TRACKER TYPE SPREAD --- */}
      <View style={styles.chartCard}>
        <Text style={styles.cardLabel}>📂 STRUKTUR TABURAN FORMAT SOALAN</Text>
        <View style={styles.distributionContainer}>
          <View style={styles.distRow}>
            <Text style={styles.distText}>🔤 Soalan Format Teks Ringkas:</Text>
            <Text style={styles.distCount}>{chartData.textCount || 0} Item</Text>
          </View>
          <View style={styles.distRow}>
            <Text style={styles.distText}>🖼️ Fail Lampiran Multimedia (Imej/Video/PDF):</Text>
            <Text style={styles.distCount}>{chartData.mediaCount || 0} Item</Text>
          </View>
        </View>
      </View>

      {/* --- HERO SCORE CARD WINNING POST VIEW CONTAINER --- */}
          ...
          <View style={styles.cardContent}>
            ...
            {stats.topQuestion.type === 'image' && (
              <View style={styles.mediaContainer}>
                <Image source={{ uri: stats.topQuestion.content }} style={styles.analyticsMedia} />
            ...
            {stats.topQuestion.type === 'video' && (
              <View style={styles.mediaContainer}>
                <Video source={{ uri: stats.topQuestion.content }} style={styles.analyticsMedia} useNativeControls />
                <Text style={styles.mediaSubtext}>
            ...
            {stats.topQuestion.type === 'file' && (
              <View style={styles.mediaContainer}>
                ...
                <Text style={styles.mediaSubtext2}>{stats.topQuestion.text || "Dokumen PDF Teratas"}</Text>
              </View>
            )}
      ...
      {/* --- RETREAT COMMAND DISMISS BUTTON --- */}
...
const styles = StyleSheet.create({
  ...
  // CONTROLLED WIDTH BOUNDS MATRIX
  statsRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20, alignSelf: 'center', width: '100%', maxWidth: 600 },
  ...
  chartCard: { backgroundColor: '#FFF', padding: 15, borderRadius: 18, marginBottom: 20, elevation: 2, alignSelf: 'center', width: '100%', maxWidth: 600 },
  ...
  // SECURED LAPTOP MONITORS DISPLAY LIMITER BOUNDARIES
  topQuestionCard: { 
    backgroundColor: '#FFF', 
    padding: 20, 
    borderRadius: 18, 
  ...
  // EMBEDDED CROPPING LOGIC SYSTEM
  mediaContainer: { 
    alignItems: 'center', 
  ...
  mediaSubtext2: { fontSize: 12, color: '#3498DB', fontWeight: '700', marginTop: 8, textAlign: 'center' },
  analyticsPdfBtn: { backgroundColor: '#F39C12', paddingVertical: 12, borderRadius: 10, alignItems: 'center', width: '90%' },
  ...
  doneBtn: { backgroundColor: '#2C3E50', paddingVertical: 15, borderRadius: 12, alignItems: 'center', alignSelf: 'center', width: '100%', maxWidth: 600 },
