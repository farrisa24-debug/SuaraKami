...
const FeedScreen = ({ pin, onBack }) => {
  ...
  const [myVotes, setMyVotes] = useState({}); // Tracking: { postId: count }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.postBubble}>
            ...
            {item.type === 'video' && <Text style={styles.mediaLabel}>📹 Video Clip Sedia</Text>}
            {item.type === 'file' && <Text style={styles.mediaLabel}>📄 Dokumen PDF</Text>}
      ...
      <View style={styles.inputBar}>
        ...
        <TextInput style={styles.input} placeholder="Tanya sesuatu..." value={message} onChangeText={setMessage} />
...
const styles = StyleSheet.create({
  ...
  mediaLabel: { color: '#3498DB', fontWeight: 'bold', fontSize: 12 },
